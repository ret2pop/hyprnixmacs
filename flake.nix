# [[file:../config/nix.org::*Flake.nix][Flake.nix:1]]
{
  ...
  '';
        ...
        # parse the lockfile directly as pure JSON data
        lockfile = builtins.fromJSON (builtins.readFile ./flake.lock);
        
        # extract only the nodes that actually contain locked source trees
        lockedNodes = builtins.filter (node: node ? locked) (builtins.attrValues lockfile.nodes);
        
        # native schema resolution
        allFlakeSources = builtins.map (node:
          (builtins.fetchTree (builtins.removeAttrs node.locked ["dir"])).outPath
        ) lockedNodes;

        # bundle them securely into a native Nix derivation
        flakeSourcesClosure = pkgs.stdenvNoCC.mkDerivation {
        ...
        mkInstallerTests = builtins.map (hostname:
          ...
          in {
            ...
            value = if (isVirtual || isSdCard) then
              ...
              ''
                      ...
                      # only evaluate if bare-metal desktop host
                      targetSystem.pkgs.testers.runNixOSTest {
                        ...
                        nodes.installer = { ... }: {
                          ...
                          # Inject all dependencies for the installer during the VM build time; VM has no WAN access for reproducibility reasons.
